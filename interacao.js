...
let valores = [] //criação de um vetor
//O queryselect funciona por id de tags


function isNumero(n) {
    if(Number(n) >= 1 && Number(n) <= 100) { //se o numero n estiver entre 1 e 100
        return true
    } else {
        return false
    }
...
function finalizar() {
    if (valores.length == 0) {
        ...
        res.innerHTML += `<p>${media} é a média dos números digitados.</p>`
    }
}